...
### 宏觀戰情室與 AI 智慧每日作戰方針決策架構 *(新增於 V8.13.0 / ADR #0094)*

- **Portfolio Macro Shield (個人投資組合宏觀防護盾)**:
  - 整合實質現金購買力比率（緊繃 `<10%`、適中 `10%~30%`、充裕 `>30%`）、質押借貸維持率安全等級（安全 `\ge 166%`、警戒 `130%~166%`、追繳 `<130%`、零借款 `NO_LOAN`）與資產配置最大偏離度。
- **Upcoming Financial Catalysts (關鍵財經催化劑倒數計算)**:
  - 動態計算 FOMC、美國 CPI、非農與央行理監事會議之倒數天數，提前預警市場波動外溢。
- **Dual-Track AI Strategy Advisor Engine (雙軌制 AI 每日作戰方針引擎)**:
  - **軌道一（100% 離線確定性專家規則系統）**：
...
### 三色導航儀動作歸併對齊、借貸卡片雙欄佈局與未到除息日預估股息動態重算 *(新增於 V8.20.0 / ADR #0101)*

- **Traffic-Light Action Matrix Alignment (三色導航儀動作歸併與總數對齊)**:
  - 核心機制：在紅黃綠交通燈號體系下，達瓦斯箱體常態整理之 `HOLD`（區間觀望）標的統一歸併入黃燈「觀望待變／不碰」陣營。
  - 效益：篩選列與導航儀黃色區塊之標的數量精確統計，達成「建議買進 + 觀望待變 + 建議賣出 = 全部標的」100% 精準對齊（如 $10 + 5 + 1 = 16$），徹底消除遺漏標的與數字不一致的體驗缺陷。
- **Loan Grid Two-Column Layout (進行中股票質押與借款雙欄響應式佈局)**:
  - 核心機制：採用 `repeat(auto-fit, minmax(min(100%, max(360px, calc(50% - 16px))), 1fr))` 網格約束。
  - 效益：寬螢幕或 1080p 桌面環境下一列最多只並排 2 個卡片，賦予質押卡片充足的 500px+ 展示空間，消除 3 欄時本利和與規費緊湊折行問題；小螢幕平滑降級為單欄。
- **Pending Dividend Dynamic Recalculation Engine (未到除息日預估股息依最新持股動態重算引擎)**:
  - 核心模組：`src/engine/cashLedgerEngine.ts` 之 `reconcilePendingDividendTrades`。
  - 核心規則：
    - 除權息的在籍持股依法規以除息日前一日（`exDate - 1 day`）收盤為準。
    - 若尚未到達除息日（`today < exDate`），投資人若在除息日前變更買賣持股，原補登之除息 TradeRecord 與待交割現金流水**自動依最新在籍持股動態重新計算股數、每股配息淨額與二代健保補充保費**（如台積電 2330）。
...
### 肌肉書僮風益比硬門檻、帶寬方向確立與無效代碼防護架構 *(新增於 V8.24.0 / ADR #0105)*

- **Risk-Reward Hard Gate (風益比硬門檻 R:R >= 2.0)**:
  - 核心機制：`muscleBookerEngine.ts` 在突破箱頂與破底翻買點全面導入風益比檢核。
  - 判定規則：
    - 突破箱頂：計算突破後之等幅滿足目標與箱頂防守線之風益比；
    - 破底翻：計算向上至箱型頂部壓力之利潤空間與向下防守箱底支撐之停損風險。
    - 若風益比不足 $2.0R$（如 1599 宏佳騰向上利潤僅 $0.1 元、防守停損 $0.85 元，風益比僅 0.1R），系統強制降級為 `HOLD`（觀望待變），主理由標註向上空間狹窄，杜絕高風險雞肋買點；僅在 $R:R \ge 2.0$ 時給予 `BUY` 決策。
- **Bandwidth Extreme Contraction & Direction Gate (布林帶寬極致壓縮與方向確定性審查)**:
  - 核心機制：當布林帶寬 $\le 8\%$ 時，標示為極致壓縮狀態，前置判定為 `AVOID`（方向未明，嚴禁猜測押注）。
  - 均線反壓：當 20MA 明顯下彎蓋頭反壓時，前置判定為 `AVOID`（均線蓋頭，反壓沉重），嚴格避免逆勢承接。
- **Risk-Reward Priority Sorting (買進推薦池依風益比降序排列)**:
  - 核心機制：導航儀「建議買進」推薦池全面依 `riskRewardRatioValue` 降序排列，優先將 5.3R ➔ 3.2R ➔ 2.1R 等高盈虧比標的置頂展示，並在卡片標註金色火焰高光 `🔥 風益比: 1:XR`。
...
### 肌肉書僮風益比全面統一專業 R 倍數規範 *(新增於 V8.29.0 / ADR #0110)*

- **Universal R-Multiple Standard (全域 R 倍數制標準)**:
  - 核心定義：全面廢除 `1 : X` 與 `X : 1` 比例混用造成之方向顛倒矛盾，亦徹底消除 `1:7.9R` 等冗贅語病。
...
### 聰明錢動態星圖盤中有效日報回溯機制與質押法定沖償引擎 *(新增於 V8.36.0 / ADR #0117)*

- **Last Known Good Institutional Report Fetcher (盤中籌碼日報回溯與有效日判定)**:
  - 核心痛點：盤中（15:30 證交所三大法人日報結算前）或週末連假時，證交所未公布當日籌碼，舊系統因無資料 fallback 為 0，導致動態星圖外資/投信/自營商買賣超為 0 張、氣泡在中軸一字排開。
  - 盤中時間感知切換：`smartMoneyFetcher.ts` 判斷當前台灣時間若早於 15:30，優先請求前一營業日 ($T-1$) 籌碼日報；15:30 後才嘗試請求當日最新日報。
  - 本地快取歷史回溯：若遠端日報為空或網路異常，自動依序往前回溯本地最近 5 個交易日之日報，以權值哨兵 `2330` 具有三大法人數據判定為有效日報（Valid Report），杜絕全 0 橫排。
  - 狀態結構與真實交易日時間軸：回傳 `InstitutionalReportResult`（包含 `data`, `reportDate`, `isFallback`, `statusBadge`），介面以顏色徽章清楚標註資料結算基準日，時間軸動態跳過週末與國定假日。
- **Statutory Debt Repayment Priority Engine (質押借貸「規費 ➔ 利息 ➔ 本金」法定沖償順序)**:
  - 核心痛點：原質押還本強制將 `lastInterestPaymentDate` 推進至當天，導致前段尚未清償之利息蒸發少算一天；且合約既有規費未抵扣歸零，次日重複計收 60 元。
  - 法定沖償順序 (民法第 323 條)：`applyDebtRepayment(loan, repaymentAmount, paymentDate)` 依「① 待繳規費 ➔ ② 累積未結利息 ➔ ③ 借貸本金」順序精確扣抵。
  - 規費清零抵扣：若還款覆蓋了合約待繳規費（如集保解質撥券費 60 元），合約規費立即結清歸零，次日利息計算不再重複收費。
  - 計息日起算日推進守護：僅在還款足以「全額清償未結利息」時，才將 `lastInterestPaymentDate` 推進至還款日；若利息未全額清償，計息起算日維持不變，杜絕利息漏算。
...
### 個股分析成長率基期補齊、極端值視覺防禦、真實河流圖與估值模型審計 *(新增於 V8.46.2 / Spec #0131 / Issue #61)*

- **Growth Baseline Expansion & Baseline Flattener (成長率基期擴充與缺失平整化)**:
  - 核心機制：API 抓取起始日擴充至 `2021-01-01`，補齊 2022 年 4 季之完整前期同期基期；當因資料庫剛上市或其他因素導致缺乏前期基期時，嚴格輸出 `displayValue: '-'` 並將柱高歸零，徹底杜絕誤導投資人為持平的 `0%` 假綠柱。
- **Outlier Visual Capping Engine (離群極端值自適應可視封頂防禦)**:
  - 核心機制：針對台泥 23Q2 毛利暴衝 `+5326.2%` 等歷史低基期爆發事實，提取非離群值（$<300\%$）上限作為正常季度縮放基準，使常態季度（如 +14.5%, +52.1%）保有真實起伏與可比性；極端值高度封頂於 100%，並標註 `⚡+5326.2%` 與低基期效應說明，兼顧數據真實性與圖表走勢美感。
- **True SVG Valuation River Bands Engine (真實 SVG 估值河流圖引擎)**:
  - 核心機制：揚棄寫死 18% 藍色假方塊，改以 SVG 多邊形 (`<polygon>`) 繪製 4 階彩色漸層河流帶（超跌便宜、合理偏低、合理偏高、昂貴），隨各季真實獲利（TTM EPS、每股淨值 BVPS、平均股息）動態波動；疊加即時現價線、最新落點脈衝光點與評級徽章（如 `落點評估：合理偏高 (18x ~ 22x)`），並提供歷季通道價位數據對照矩陣。
- **Comprehensive Redesign of 7 Valuation Subtabs (價值評估 7 大獨立子頁面重構)**:
  - `pe_valuation` (本益比評價)：滾動近 4 季 TTM EPS、五檔本益比通道階梯卡片、現價評估與 P/E 分布。
  - `pe_river` (本益比河流圖)：真實 SVG 本益比河流圖。
...
- **Quant Cards Dynamic Weighting & Signal Synthesis (中間量化卡片群 Card 10~17 動態加權與信號合成)** *(新增於 V8.56.0 / Spec #0143 / Issue #105)*:
  - **Card 10 (bullBearEnergy)**：以近 20 日紅黑 K 成交量動態計算多空能量百分比與多空比值（`bullBearRatio`），具備黑 K 為 0 時自動封頂 `99.99` 之安全除零防護。
  - **Card 11 (healthSummary)**：整合籌碼健康、均線多頭排列度、資金動能、流動性風險與法人支撐力 5 維度動態評分，綜合計算整體評等標籤（優良/良好/普通/偏弱）。
  - **Card 12 (dynamicSignals)**：依據最新收盤價相對 20 日 VWAP 主力成本乖離率、KD、RSI 與法人買賣超動態生成趨勢/籌碼/動能信號；破位警戒時亮 `RED`、多頭順風時亮 `GREEN`、其餘維持 `YELLOW`。
...
### AI 主力戰情室卡片 06 與 07 照片精準對齊、動態波形面積圖與雙色發散錐 *(新增於 V8.57.0 / Spec #0144 / Issue #109)*

- **Split Forecast Cone Geometry & Price Y-Axis (卡片 06 雙色發散扇形錐與價格座標軸體系)**:
  - 核心機制：透過 `buildSplitConePaths` 以中央中位數曲線 (`medianPoints`) 為幾何分界，向上與上邊界 (`upperPoints`) 閉合形成紅色/橙色漸層「上漲發散扇形」；向下與下邊界 (`lowerPoints`) 閉合形成翠綠色漸層「下跌發散扇形」，直觀對齊圖例的多空機率。
  - 座標與標籤：左側動態計算極值並渲染價格數值刻度（如 2500, 2000）與水平參考格線；主力方向機率標籤依多空動態映射色彩（多頭為鮮紅色 `#ef4444`）。

- **Dynamic Stacked Mountain VWAP Cost Structure (卡片 07 多時段多層堆疊波形面積圖)**:
  - 核心機制：在 `vwapCostEngine` 中建立時間序列採樣演算法，基於近 60 日歷史日 K 採樣 4 個時點（如 06/25, 07/10, 08/10, 08/31），計算各時點相對於當時 20 日 VWAP 的 4 階成本帶成交量分佈（倉儲區 >5%、套牢區 -2~-5%、主力成本區 ±2%、大量成交區 ±2~5%）。
  - SVG 平滑堆疊路徑：由 4 階累積高度透過三次貝茲曲線平滑閉合，呈現「橙黃 ➔ 翠綠 ➔ 天藍 ➔ 深藍」之起伏山峰；右上圖例、Y 軸量能刻度 (60k~0k)、標題校正為「07 主力成本結構分佈圖」，強弱指標色彩對齊天藍色。

- **Financial Terminology Precision (卡片 09 財經術語標準化)**:
  - 核心校準：隔日沖風險分析卡片第 2 項指標文案由「籌碼過手率」精準校正為標準金融術語「**籌碼換手率**」。

### AI 主力戰情室響應式 Bento-Grid 重構與雙層繁中行情列 *(新增於 V8.58.0 / Spec #0145 / Issue #112)*

- **Two-Tier Header Market Bar (雙層全功能即時行情列)**:
  - **Upper Bar (操作輸入與系統狀態)**：左側整合大字體股票代號輸入框、股票名稱與「分析」按鈕；右側橫排 4 大全繁體中文科技感狀態膠囊燈號（`AI 智慧掃描` 帶呼吸燈、`主力行為追蹤`、`市場即時狀態`、`波動異常預警`），徹底消滅英文術語與視覺擠壓。
  - **Lower Bar (寬幅即時行情大面板)**：今日收盤價（1.3rem 特大粗體）、今日漲跌/漲幅（高對比雙色發光）、成交量(張)、成交筆數、開盤、最高、最低、最新交易日、資料筆數整齊橫排，告別橫向捲軸。

- **Full-Width Row 1 K-Line Stage (01 主 K 線獨立全寬滿版)**:
  - 將 `01 主 K 線圖` 抽離原本 5 卡擁擠 row，單獨設為第 1 排（100% 滿版獨立寬度），賦予完整橫向呼吸視野與專業看盤深度，週期與副圖指標自由切換不受側邊卡片擠壓。

- **Content-Driven Sequential Bento-Grid (01➔18 內容導向自然流 Bento-Grid)**:
  - 突破死板 4 行限制，依據每張卡片的圖表形態與資訊密度量身配置專屬寬度權重，且嚴格維持 01 ➔ 18 由左至右、由上而下自然順序：
    ...
    - **Row 3**：`06, 07, 08` (3 卡，`08 法人行為計量` 獨享 1.8fr 寬幅，柱狀圖與 3 日表徹底隔開)
...
### AI 主力戰情室「03 多維度判讀」、「13 市場情緒」與「14 AI 信心度」全量化資料管線動態整合 *(新增於 V8.59.0 / Spec #0146 / Issue #115)*

- **Multi-Dimension Radar Quant Engine (卡片 03 多維度判讀 6 軸量化引擎)**:
  - 核心機制：透過獨立純函式模組 `multiDimensionRadarEngine`，基於真實歷史日 K、技術指標、主力成本與三大法人歷史記錄動態推算 6 大軸向分數（0~100）：
    - **法人 (`institutional`)**：近 20 日法人累計買賣超與土洋合買/齊賣加減分，美股無資料平滑回退 50 基準分。
    - **趨勢 (`trend`)**：均線多頭排列度 (MA5>10>20>60) 與收盤價相對 VWAP 主力成本之乖離率修正。
    - **籌碼 (`chips`)**：近 5 日法人合計量能變化與大戶集中度。
    - **流動 (`liquidity`)**：5 日 / 20 日均量門檻映射（台股大於 3000 張獲 88+ 分，小於 300 張獲 25 分）。
    - **波動 (`volatility`)**：20 日日波幅標準差，健康波動 (1.0%~4.0%) 獲 85 高分，極端劇烈 (>6.0%) 或死水盤 (<0.5%) 折算得分。
    - **動能 (`momentum`)**：KD(9,3,3) 黃金交叉、RSI(14) 區間與當日成交量放大倍數。
    - **等級判定**：6 軸等權平均取整，`>=80: 'A'`、`>=65: 'B'`、`>=50: 'C'`、`<50: 'D'`，徹底消除切換股票時卡死在 56 分 C 級之問題。

- **AI Confidence 4-Metric Engine (卡片 14 AI 信心度 4 大指標量化引擎)**:
  - 核心機制：透過獨立模組 `aiConfidenceEngine`，依據樣本充足度與訊號品質動態推算：
    - **資料完整度 (`dataCompleteness`)**：日 K 數列長度 (>=120 根且有法人為 98%)。
