...
- **實拍像素級 4 排 Bento-Grid 矩陣佈局**：
  - 完整重現頂級量化交易戰情室介面，重構為精確的 4 排網格矩陣（Row 1: 5卡；Row 2: 4卡；Row 3: 6卡等寬；Row 4: 3卡）。
  - **頂部雙列整合 Bar**：第 1 列整合 4 大狀態指示膠囊（`AI SCAN ACTIVE`、`MAIN FORCE TRACKING`、`MARKET STATUS`、`VOLATILITY ALERT`）、發光股票代號輸入框、股票名稱、`[分析]` 按鈕與 10 大即時行情指標；第 2 列左側資料來源說明、右側對齊 5 大匯出按鈕。
  - **圖形升級**：垂直價格階梯熱力矩陣 (1600~2400) 與 5 大量化分佈區間、平滑三次貝茲波形堆疊圖 (Wave Area Stack)、水平雙條對比能量棒（多空比 1.13 倍多）、並排 3 個獨立進度環 (大戶買盤 65% / 散戶買盤 35% / 散戶賣盤 36%)、全 18 張卡片右上角統一配備微型選單 `⋮`。
...
- **原生 Glassmorphism 擬態與排版防禦 (`Native Glassmorphism & Anti-Break Layout`)**：
  - 徹底移除失效之 Tailwind 類別，改採純原生 Inline Styles 與專案 Design Tokens，徹底根除白色破版與透明卡片問題。
  - SVG 儀表板指針錨定 ViewBox (`0 0 200 135`) 與旋轉原點，徹底消滅巨大黑三角箭頭遮擋。
  - 穿透式詳細報告彈窗 (`HealthReportModal`) 深色毛玻璃化 (`backdropFilter: blur(12px)`)，高對比綠勾與紅叉呈現，支援 `ESC` 鍵全域監聽。
- **ETF 智慧識別防呆橫幅 (`ETF Smart Guard Banner`)**：
  - 建立 `isEtfSymbol` 引擎，智慧識別台股 `00...`（含 `00403A` 等混合編號）與美股大盤 ETF。
  - 當前選中標的為 ETF 時，頂部浮現琥珀色毛玻璃防呆橫幅，主動說明 ETF 為一籃子資產組合不適用企業財報量化分析，並提供一鍵切換個股按鈕。
  - 預設標的優先選擇持倉普通股，庫存全為 ETF 時安全預設為熱門標的 `2330`。
- **Stock Pills 快捷標的膠囊列 (`Stock Pills Navigation`)**：
  - Header 下方橫向滾動膠囊列整合持倉個股與 6 大熱門權值標的（2330 台積電、2454 聯發科、2317 鴻海、NVDA、AAPL、IBM）。
  - 內建 `seen` Set 雙重去重防護，高亮發光選中狀態，一秒無縫切換。
- **說明橫幅開合狀態記憶 (`Banner State Persistence`)**：
  - 頂部科技海軍藍漸層橫幅支援展開與收起，開合狀態即時持久化於 `localStorage`。

### 0. 股票健診系統純運算診斷引擎、穿透報告與專屬工作區 (`Stock Health Check Diagnosis Engine, Forensic Report & Dedicated Workspace`) *(V8.45.0)*
...
- **市場狀態機與多空矛盾懲罰 (`evaluateMarketRegime` & `Contradiction Penalty`)**：
  - 精確識別 `VOLATILITY_SQUEEZE` (⚡變盤在即)、`CHOPPY_RANGE` (〰️無趨勢盤整)、`TRENDING_BULL` (🚀強多主升) 與 `TRENDING_BEAR` (🔻空頭主跌)。
  - 當 `ADX < 20` 施加無趨勢鈍化折扣；當均線多排但 `-DI > +DI` 時，扣除 20 分並強制將總分封頂於 58 分 (`NEUTRAL`)，徹底杜絕無趨勢橫盤誤判 95 分的高危盲點。
- **關鍵位密集聚集演算法 (`calculateKeyLevelClusters`)**：
  - 自動將相距 $\le 1.5\%$ 的 Pivot、Fibonacci、布林軌道、Darvas 箱體聚合成「第一壓力帶」與「防守帶」，一眼看清阻力共振區。
- **真實波幅動態吊燈防守 (`ATR(14) Chandelier Exit`)**：
  - 以 Wilder ATR(14) 計算動態移動停損點（22日最高價 $- 2.5 \times \text{ATR}$），隨股價波動自適應調整。
- **背離偵測與誘多假突破 (Divergence & Bull Trap Detector)**：
  - 搜尋近 20 根 K 線波峰波谷，比對頂背離倒貨警訊；在壓力區偵測長上影線墓碑倒錘線之假突破 (Bull Trap)。
- **三層漸進式決策 UI (Progressive Disclosure)**：
  - 【第 1 層：0秒決策核心】：頂部 Hero 卡片，顯示校正後評分、市場狀態徽章與大白話操盤指南。
