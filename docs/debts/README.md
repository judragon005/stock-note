...
| [**0019**](0019-local-historical-indicators-and-external-backfill-engine.md) | 本地全量歷史技術指標庫與免費外部資源自動回補引擎 (含肌肉書僮量化體系) | `P2` | `OPEN` | /grill-with-docs 需求調研 | `Quant` · `Indicators` · `MuscleBooker` · `Storage` | 開發 K 線圖表或肌肉書僮短線選股時 |
| [**0020**](0020-market-war-room-macro-liquidity-and-ai-advisor.md) | 宏觀戰情室、全球流動性監控與 AI 智慧每日操作決策儀表板 | `P2` | `OPEN` | /grill-with-docs 需求調研 | `Architecture` · `Macro` · `WarRoom` · `AI-Advisor` · `Dashboard` | 開發戰情室頁籤或宏觀數據串接時 |
...
| [**0025**](0025-ex-dividend-and-black-swan-margin-stress-matrix.md) | 除權息與黑天鵝多維動態壓力測試矩陣與斷頭逃生模擬器 | `P1` | `OPEN` | /grill-with-docs 需求調研 | `Architecture` · `Risk` · `Margin` · `Pledge` · `StressTest` | 質押風控升級或市場進入極端波動時 |
...
| [**0027**](0027-dual-momentum-and-relative-strength-rotation.md) | 雙重動能與跨資產趨勢輪動評分引擎 | `P2` | `OPEN` | /grill-with-docs 需求調研 | `Quant` · `DualMomentum` · `AssetAllocation` · `Strategy` | 開發輪動雷達或整合宏觀戰情室時 |
