...
1. **代碼重構**：依序重構 `FinancialForensicModal.tsx`、`FinancialHeroLayer.tsx`、`FinancialTrendsLayer.tsx`、`FinancialForensicDeepAuditLayer.tsx`。
2. **自動化測試驗證**：執行 `npm test` 確認既有與新增測試全數通過。
3. **建置檢查**：執行 `npm run build` 確認生產環境 bundle 正常編譯。
