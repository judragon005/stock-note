...
- [ ] 任務二依據真實日 K 條件動態生成警示項目，不出現寫死的致茂常數。
- [ ] 任務三與任務四 KPI 數值與最新日 K (OHLCV) 之指標計算完全一致（SSOT）。
- [ ] 任務三與任務四具備原生 SVG 走勢繪製，不依賴第三方重型圖表套件。
- [ ] 任務五支援完整歷史日 K 表格呈現與簡易客戶端分頁切換。
- [ ] 單元測試 100% 綠燈，TypeScript 0 錯誤。
